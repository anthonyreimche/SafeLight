...
export function buildIccProfile(id: ColorSpaceId): Uint8Array {
  ...
  const tags: { sig: number; data: Uint8Array }[] = [
    ...
    // 'cprt' — required ICC tag. Kept neutral (no app/device identifier) so an
    // exported file's embedded profile leaks nothing about the originating app.
    { sig: 0x63707274, data: textTag("No copyright. Public domain.") },
  ];
